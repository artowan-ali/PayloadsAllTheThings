# PayloadsAllTheThings

A list of useful payloads and bypasses for Web Application Security and Pentest/CTF.

## Summary

This repository is a collection of payloads and methodologies for web application security testing, bug bounty hunting, and CTF challenges.

## Categories

- [API Key Leaks](api-key-leaks/README.md)
- [Command Injection](command-injection/README.md)
- [CORS Misconfiguration](cors-misconfiguration/README.md)
- [CSRF Injection](csrf-injection/README.md)
- [Directory Traversal](directory-traversal/README.md)
- [File Inclusion](file-inclusion/README.md)
- [File Upload](file-upload/README.md)
- [GraphQL Injection](graphql-injection/README.md)
- [Insecure Deserialization](insecure-deserialization/README.md)
- [JWT Attacks](jwt-attacks/README.md)
- [LDAP Injection](ldap-injection/README.md)
- [NoSQL Injection](nosql-injection/README.md)
- [OAuth Misconfiguration](oauth-misconfiguration/README.md)
- [Open Redirect](open-redirect/README.md)
- [Prototype Pollution](prototype-pollution/README.md)
- [Race Condition](race-condition/README.md)
- [Request Smuggling](request-smuggling/README.md)
- [Server Side Request Forgery](server-side-request-forgery/README.md)
- [Server Side Template Injection](server-side-template-injection/README.md)
- [SQL Injection](sql-injection/README.md)
- [SSRF](ssrf/README.md)
- [Subdomain Takeover](subdomain-takeover/README.md)
- [Upload Insecure Files](upload-insecure-files/README.md)
- [Web Cache Deception](web-cache-deception/README.md)
- [Web Sockets](web-sockets/README.md)
- [XPATH Injection](xpath-injection/README.md)
- [XSS Injection](xss-injection/README.md)
- [XXE Injection](xxe-injection/README.md)

## Contributing

Pull requests are welcome. For major changes, please open an issue first to discuss what you would like to change.

Please read [CONTRIBUTING.md](CONTRIBUTING.md) for details on our code of conduct and the process for submitting pull requests.

## License

This project is licensed under the MIT License - see the [LICENSE](LICENSE) file for details.

## Acknowledgements

- Original project by [swisskyrepo](https://github.com/swisskyrepo/PayloadsAllTheThings)
- All contributors who have submitted payloads and improvements
